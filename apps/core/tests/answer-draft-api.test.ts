...
describe("GET /internal/status", () => {
  it("returns a consolidated internal service status snapshot", async () => {
    ...
    expect(response.json()).toEqual({
      ...
      summary: {
        ...
        disabledComponentCount: 4,
        disabledComponents: ["answerDraft", "memoryExtraction", "actionApprovals", "proactiveSignals"],
  ...
  it("keeps the consolidated status available when one component status fails", async () => {
    ...
    expect(response.json().summary).toEqual({
      ...
      disabledComponents: ["answerDraft", "memoryExtraction", "reindex", "actionApprovals", "proactiveSignals"],
...
describe("authorized wiki document registration API", () => {
  ...
  it("rejects comma-contaminated authorized wiki document URLs before registration", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/internal/document-sync/authorized-wiki-documents",
      payload: {
        sourceUri: "https://docs.feishu.cn/docx/doc_token_1,please",
        authorizedSpaceId: "space-1",
      },
    });
