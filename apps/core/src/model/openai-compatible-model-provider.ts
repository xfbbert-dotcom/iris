...
const COMMON_ANSWER_DRAFT_SYSTEM_PROMPT_PARTS = [
  ...
  "When the current Question gives substantive feedback about tone, style, wording, or a previous response, acknowledge the feedback directly and adapt this reply instead of treating it as background fact.",
  "When the Question or reference Context includes a diary, story, roleplay, quotation, or another person's first-person account, treat its narrator as supplied text, not Iris's experience or the user's personal history.",
  ...
  "Ground company-specific factual claims only in authorized evidence. When required company evidence is absent, say what is unavailable instead of guessing or using general world knowledge to fill the gap.",
...
const CONTEXTUAL_ANSWER_DRAFT_SYSTEM_PROMPT = [
  ...COMMON_ANSWER_DRAFT_SYSTEM_PROMPT_PARTS,
...
const STANDALONE_ANSWER_DRAFT_SYSTEM_PROMPT =
  COMMON_ANSWER_DRAFT_SYSTEM_PROMPT_PARTS.join(" ");
...
export function createOpenAICompatibleModelProvider(
  ...
  return {
    async generateAnswerDraft(input) {
      ...
      const isStandalone = input.contextMode === "standalone";
      if (!isStandalone) {
      ...
      const parsedAnswer = parseAnswerContent(await chatClient.complete(
        isStandalone
          ? [
              { role: "system", content: STANDALONE_ANSWER_DRAFT_SYSTEM_PROMPT },
              { role: "user", content: `Question:\n${input.question}` },
            ]
          : [
              { role: "system", content: CONTEXTUAL_ANSWER_DRAFT_SYSTEM_PROMPT },
              { role: "user", content: `Context:\n${input.promptContext}` },
              { role: "user", content: `Question:\n${input.question}` },
            ],
      ));
      if (isStandalone && parsedAnswer.citedSourceRefs !== undefined) {
        throw new Error("standalone model response included a document reference");
      }
