...
async function claimApprovedUpdate(dataSource: PostgresKnowledgeDraftDataSource, input: ClaimManagedUpdateInput): Promise<ClaimedManagedKnowledgeUpdate> {
  ...
  return withTransaction(dataSource, async (client) => {
    ...
    if (page.version !== normalized.expectedManagedPageVersion || page.state !== "active" || page.linkedDocumentSourceId !== target.linkedDocumentSourceId) throw new ManagedKnowledgePageVersionConflictError();
...
async function recordRemoteOutcome(dataSource: PostgresKnowledgeDraftDataSource, input: RecordManagedRemoteOutcomeInput): Promise<ManagedExecutionMutationResult> {
  ...
  return withTransaction(dataSource, async (client) => {
    ...
    if (execution.version !== normalized.expectedExecutionVersion) throw new ManagedKnowledgePageVersionConflictError();
    const pageState = normalized.classification === "remote_applied" ? "resync_required" : normalized.classification === "outcome_unknown" || normalized.classification === "reconciliation_required" ? "reconciliation_required" : normalized.classification === "preflight_failed" && page.state === "updating" ? "active" : page.state;
...
async function completeResync(dataSource: PostgresKnowledgeDraftDataSource, input: CompleteManagedResyncInput): Promise<ManagedExecutionMutationResult> {
  ...
  return withTransaction(dataSource, async (client) => {
    ...
    if (execution.version !== normalized.expectedExecutionVersion || page.state !== "resync_required" || observation.managedPageId !== page.id || observation.managedBodyContentHash !== execution.afterBodyContentHash || observation.observedRemoteRevisionId !== execution.responseRevisionId) throw new ManagedKnowledgePageVersionConflictError();
