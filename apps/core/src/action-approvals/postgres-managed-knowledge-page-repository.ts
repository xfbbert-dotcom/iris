...
const targetColumns = ["id", "draft_id", "draft_revision", "draft_version", "conflict_candidate_id", "conflict_candidate_version", "managed_page_id", "managed_page_version", "linked_document_source_id", "target_snapshot_id", "target_snapshot_hash", "target_source_version", "remote_document_token", "managed_body_block_id", "expected_remote_revision_id", "current_body_content_hash", "proposed_body_content_hash", "authorization_group_id", "target_policy_id", "target_policy_version", "operation_key", "operation_fingerprint", "created_at"] as const;
function targetSelect(prefix = ""): string { return `SELECT ${targetColumns.map((column) => `${prefix}${column}`).join(", ")} FROM knowledge_publication_update_targets`; }
