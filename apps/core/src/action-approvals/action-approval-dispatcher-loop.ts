import type { ActionApprovalDispatcherResult } from "./action-approval-dispatcher.js";

type TimerHandle = ReturnType<typeof setTimeout>;
const MAX_TIMER_DELAY_MS = 2_147_483_647;
const MAX_BATCH_LIMIT = 100;

export type ActionApprovalDispatcherBatchSnapshot =
  | {
      status: "succeeded";
      startedAt: Date;
      finishedAt: Date;
...
export type ActionApprovalDispatcherLoopSnapshot = {
  running: boolean;
  intervalMs: number;
  batchLimit: number;
  latestBatch?: ActionApprovalDispatcherBatchSnapshot;
};

export function createActionApprovalDispatcherLoop({
  worker,
  intervalMs,
  batchLimit,
  onError,
  ...
  worker: { processBatch(input: { limit: number }): Promise<ActionApprovalDispatcherResult[]> };
  intervalMs: number;
  batchLimit: number;
  ...
  let latestBatch: ActionApprovalDispatcherBatchSnapshot | undefined;
  ...
  return {
    ...
    getSnapshot(): ActionApprovalDispatcherLoopSnapshot {
      return {
        running,
...
function count(
  results: ActionApprovalDispatcherResult[],
  status: ActionApprovalDispatcherResult["status"],
): number {
  return results.filter((result) => result.status === status).length;
}

function cloneSnapshot(snapshot: ActionApprovalDispatcherBatchSnapshot): ActionApprovalDispatcherBatchSnapshot {
  return {
    ...snapshot,
...
function requireDate(value: Date): Date {
  if (!(value instanceof Date) || Number.isNaN(value.getTime())) {
    throw new Error("action approval dispatcher loop time must be valid");
  }
