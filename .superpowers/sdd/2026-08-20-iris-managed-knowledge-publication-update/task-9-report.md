...
## Fix Round 2/5 (exact reconciliation replay)

### Implemented

- Scoped the reconciliation-request operation fingerprint to its durable operator intent: execution id, expected execution/page versions, operation key, and operator. It intentionally excludes server-generated audit `at`; all other remote-outcome mutation fingerprints retain their existing behavior.
- Added a content-free durable reconciliation acknowledgement reconstructed from the immutable execution event (`executionId`, `reconciliation_required`, event `to_version`, `operator_requested`). Both the initial HTTP request and exact replay return this acknowledgement; only the initial request invokes the reconciler.
- Extended the configured-PostgreSQL fixture to replay the exact request with a later server time and verify immutable acknowledgement equality. It also retains semantic-key conflict coverage.

### Fix Round 2 TDD evidence

1. RED: `npm exec --workspace apps/core -- vitest run tests/postgres-managed-knowledge-page-repository.test.ts --reporter=dot` failed with the expected missing `acknowledgement` assertion (`expected ... acknowledgement ... Received`), before the durable acknowledgement implementation.
   GREEN: after the scoped fingerprint plus immutable event reconstruction, `npm exec --workspace apps/core -- vitest run tests/postgres-managed-knowledge-page-repository.test.ts tests/action-approval-runtime.test.ts tests/action-proposal-api.test.ts --reporter=dot` passed 39 tests with 1 configured-PostgreSQL skip.
2. The runtime test uses different `at` values and a first reconciler result that deliberately differs (`resync_required/readback_confirmed`); it proves first and replay responses are deep-equal durable acknowledgements and `reconcileOne` is called exactly once.
3. The API integration test builds a real `createActionApprovalRuntime` with a durable-semantics fake producer, advances the app clock between two POSTs, checks byte-equivalent response payloads, and proves the reconciler is called only once. It does not self-prove behavior by mocking the admin response.

### Fix Round 2 verification

- `npm exec --workspace apps/core -- vitest run tests/postgres-managed-knowledge-page-repository.test.ts tests/action-approval-runtime.test.ts tests/action-proposal-api.test.ts --reporter=dot` — passed: 39 tests, 1 configured-PostgreSQL skip.
- `npm exec --workspace apps/core -- tsc --noEmit` — passed (exit 0).
- `git diff --check` — passed with no whitespace errors (CRLF conversion warnings only).
- No additional full suite was run in this round, as directed; Fix Round 1's full suite was green and Task 10 owns the next `npm run verify`.

### Fix Round 2 self-review and concerns

- The reconciliation request transitions durably before any remote reconciler/readback work; exact replay exits before that remote work. No draft/document body, raw Feishu error/payload, or token is included in the acknowledgement.
- Expected versions, operator, and operation key remain validated. A changed operator or expected version with the same key is an operation conflict; later server audit time is not an operation identity.
- `IRIS_TEST_DATABASE_URL` and `DATABASE_URL` remain unavailable locally, so the real configured-PostgreSQL replay fixture is honestly skipped.
- Final Fix Round 2 read-only `git status --short` listed only this round's tracked report/source/test changes; no test-generated temporary directory was present or modified.
