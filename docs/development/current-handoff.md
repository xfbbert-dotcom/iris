...
候选`4a9f852d960e211f64b77f45f99cfb4ef5838ab4`的本机回归及[精确SHA CI34313222518](https://github.com/xfbbert-dotcom/iris/actions/runs/34313222518)已通过，包含真实PG门禁。
当前发布执行进度：候选镜像及迁移`0058`已部署，但首轮实模概括泄露“置信度为medium”。
scope已撤销到version2；05:27 UTC基础问答恢复、runtime3378、公开健康正常，共享尚未开放。
中文前缀后的英文评级修复为`ff9fab2a`，本机4329/345条件跳过及独立审查已通过，待新候选CI/实模；
不能把问卷部分通过当作整体发布成功。
...
- 之前文档同步开始时该分支为 `985af6cf`（当时`748b8404`的发布补记）；本轮候选已推进至`4a9f852d`。提交历史与运行镜像分别核对，不能把旧定位记录当作本轮基线。
...
下列为此前已完成并恢复公开入口的`748b8404`历史发布；`4a9f852d`部署、失败门禁和恢复见本页顶部，
...
当前下一步是冻结包含`ff9fab2a`的新候选，通过精确CI后重新部署与实模验收；旧候选的失败不可被略过。
