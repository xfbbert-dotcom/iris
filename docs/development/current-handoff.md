...
最终应用`f6a6dd4187dcb1b574fb78a11f837edec8b09b89`已部署并通过
[精确CI34317866518](https://github.com/xfbbert-dotcom/iris/actions/runs/34317866518)：Core4486通过/345条件跳过，真实PG及部署演练单列通过。
07:39–07:43 UTC九项内部实模检查及人工核对通过；07:43:52 UTC恢复公开入口、就绪21/21、
队列/死信/未决回复0，scope active/version5、runtime3387，精确三群普通讨论共享已开放。
两次先前候选`4a9f852d`/`9d5a5545`的语言失败及v2/v4撤销保留，不被本次成功抹去。
最终修复`d475c5d1`覆盖中文置信度双向语序；迁移0058只在第一轮应用，后续均跳过。
本轮是实际原文＋真实模型＋内部草稿验收，没有向飞书发送测试消息或新增真实回执验收。
最新进度、四处文档处置和最终结果见[专项记录](iris-shared-working-chat.md)。接下来在已批准三群日常试用，
不改写下方`748b8404`历史隔离与验收事实；Wiki写入、任务执行和主动发言等独立能力仍关闭。

## 先确认从哪里继续

- 本机已核实的实现工作树：`D:/work/AGE-org/.worktrees/iris-daily-pilot-1eb86`，分支 `codex/iris-daily-pilot-followup`。
- 之前文档同步开始时该分支为 `985af6cf`（当时`748b8404`的发布补记）；本轮已部署应用为`f6a6dd41`，后续文档提交不等于更新了运行应用。提交历史与运行镜像分别核对，不能把旧定位记录当作本轮基线。
...
下列为此前已完成并恢复公开入口的`748b8404`历史发布；本轮`f6a6dd41`成功及两个拒绝候选见本页顶部，
...
当前有限发布门禁已通过，进入精确三群日常试用，观察用户真实提问而非无限扩展本轮硬化。
出现权限/核心回答故障时按当前scope版本撤销共享并恢复安全的原有限问答；不覆盖新数据库事实。
不因本轮成功扩大群集合、开启主动发言或新的外部行动能力，也不将其说成整个产品不再有缺口。
