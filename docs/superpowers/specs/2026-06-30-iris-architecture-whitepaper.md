...
Runtime-control mutations must be auditable. Production retains the same bounded recent-event
window in PostgreSQL for runtime-control and permission diagnostics, including a durable count of
events dropped by retention. Emergency enable/disable paths must remain available even if the audit
sink is unavailable.
