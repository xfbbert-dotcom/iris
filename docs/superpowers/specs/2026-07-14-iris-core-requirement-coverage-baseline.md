...
| IRIS-CORE-007 | Iris 将讨论整理成内容，先发群里让用户确认 | Phase 5A 代码完成，端到端仍未完成 | 已有 Postgres 知识草稿、不可变修订/事件、精确证据、风险等级、状态机、当前证据脱敏、幂等与内部治理 API；Phase 5A 不调用模型、不发飞书、不进入回答检索 | 仍需真实一次性数据库治理验收，以及 Phase 5B 的自动生成、群内预览、确认/驳回/编辑和真实飞书验收；完成前不得称为已实现 |
