...
|---|---|---|---|---|
| IRIS-CORE-001 | 同一飞书群中的多人可以共同与 Iris 协作，并从前一个人的上下文继续 | 部分实现 | 所有已启用群消息进入 `conversation_messages`；回答读取最近群聊；飞书回复所有群成员可见 | 当前只有最近消息窗口，没有主题记忆、群长时记忆和行动历史 |
...
| IRIS-CORE-003 | Iris 随时间学习，用户不必重复解释业务背景 | 缺失核心实现 | 当前仅有原始消息和文档索引，可提供短时上下文与文档召回 | 需要可追踪、可删除、可纠正、按群和权限隔离的 thread/group/action memory，并接入回答顺序 |
...
1. 可持续学习的 thread/group/action 长时记忆；
2. 主动信号发现和未解决讨论跟进；
