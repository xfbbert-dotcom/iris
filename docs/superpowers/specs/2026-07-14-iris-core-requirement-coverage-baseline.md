...
| IRIS-CORE-007 | Iris 将讨论整理成内容，先发群里让用户确认 | 5B-1 与 5B-2A 已通过真实 Feishu pilot；5B-2B 代码候选完成、默认关闭 | Phase 5A Postgres 知识草稿事实层；5B-1 版本绑定群确认/修改/拒绝卡片；5B-2A `ActionProposal`、风险矩阵、目标策略、角色 grant、负责人/管理员审批卡片、实时授权、治理 API、幂等 callback 与 readiness；5B-2B 飞书 OAuth + PKCE、完整正文/哈希审阅、append-only attestation 与批准前精确门禁 | 仍需完成 5B-2B 真实 OAuth 单群验收和 5B-3 飞书知识库写入，不能称为完整端到端知识发布 |
| IRIS-CORE-008 | 用户确认后同步到飞书知识库 | 发布执行仍缺失；确认、审批与完整正文审阅前置代码已实现 | 5B-2A 已能把已确认草稿转为版本绑定 proposal 并收集 low/medium/high 风险所需批准；5B-2B 能记录绑定当前 actor/version/revision/hash 的审阅事实；`writeKnowledgeBase`、publication runtime 和 execution 仍不存在/关闭 | 仍需 5B-2B 真实 OAuth pilot，以及 5B-3 飞书写入适配器、execution、幂等、失败恢复、对账、回群结果和真实验收；未经全部批准不得写入 |
...
2. 主动信号发现和未解决讨论跟进；
3. 知识草稿事实层、5B-1 群内确认和 5B-2A 行动提案/负责人管理员审批已通过真实 Feishu pilot 并恢复默认关闭；5B-2B 完整正文审阅页代码候选已完成但真实 OAuth pilot 待验收；5B-3 知识库发布仍未实现；
4. 面向非工程管理员的轻量控制台；
5. 多公司自助安装和租户产品化。
