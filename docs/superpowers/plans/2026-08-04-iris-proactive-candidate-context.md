...
**Goal:** Show human-readable proactive candidate subjects in the Admin Console and atomically reject approval when the exact target is stale or actively suppressed.
...
- A stale or actively suppressed candidate stays visible and dismissible, but cannot be approved through either the UI or direct API.
...
**Interfaces:**
- Produces: `PendingProactiveSignalCandidate` with `approvalState: "ready" | "stale" | "suppressed"` and optional `subjectLabel`.
...
- [x] **Step 2: Run the projection tests and observe the intended failure**
...
- [x] **Step 3: Implement the minimal typed projection**
...
- [x] **Step 5: Commit the repository projection**
...
- Produces: repository results `{ status: "stale" }` and `{ status: "suppressed" }`.
- Produces: distinct HTTP 409 bodies for stale and suppressed candidates.

- [x] **Step 1: Write failing repository stale-approval tests**
...
- [x] **Step 2: Write the failing API mapping test**
...
- [x] **Step 3: Run both focused test files and observe failures**
...
- [x] **Step 4: Implement atomic target validation**

Add `stale` and `suppressed` to `approveCandidateForDelivery`'s result union. Acquire the conversation-state advisory lock, proactive-signal group advisory lock, and candidate row lock in that order. Apply the Task 1 exact-target joins and readiness predicate directly to the outbox `INSERT ... SELECT`. If no row is inserted:

1. return `already_queued` only when the unique delivery already exists and the
   exact target remains ready;
2. return `suppressed` when active group feedback blocks the candidate;
...
- [x] **Step 5: Map stale approval in Fastify**
...
Map `suppressed` to `409 proactive_signal_candidate_suppressed` as well.

- [x] **Step 6: Run both focused test files**
...
- [x] **Step 7: Commit the atomic approval contract**
...
For stale and suppressed rows, assert the explanatory text is present, the approve button is disabled, and the dismiss button remains enabled. Trigger the ready approve path and assert event history uses the subject label instead of the idempotency key. Simulate a ready-to-stale HTTP 409 race and assert the console refreshes and leaves approval disabled.

- [x] **Step 2: Run the Admin Console tests and observe failure**
...
- [x] **Step 3: Implement bounded rendering behavior**
...
Do not append `candidate.idempotencyKey` or `candidate.entityId` to any visible row or event message. Disable only the approve action when `ready` is false, distinguish suppressed from stale with accessible text, refresh on terminal approval conflicts, and leave dismissal enabled.

- [x] **Step 4: Run the Admin Console tests**
...
- [x] **Step 5: Commit the Admin Console behavior**
...
- [x] **Step 2: Run static and full Core gates**
...
- [x] **Step 3: Inspect the final diff and request independent review**
