# Iris Authorized Wiki Document Registration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an internal API that registers one authorized Feishu wiki/document source and enqueues it for sync.

**Architecture:** Extend `DocumentSyncRuntime` with `registerAuthorizedWikiDocument`, backed by the existing document source registry and manual sync planner. Add a Fastify route that validates `{ sourceUri, authorizedSpaceId, title? }` and returns the registered source plus enqueue result.
...
Assert `runtime.registerAuthorizedWikiDocument({ sourceUri, authorizedSpaceId, title, observedAt })` calls `documentSources.registerAuthorizedWikiDocument` and then `manualPlanner.enqueueSource`.
...
Add `POST /internal/document-sync/authorized-wiki-documents`.
...
Append Phase 3H summary and verify PR #3 remains open and non-draft.
