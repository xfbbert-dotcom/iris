...
if ($KnownGroupIds -contains $PilotGroupId) { throw "Pilot group must not be a negative-control group" }
...
$redisCounts = @(& docker @compose exec -T redis redis-cli LLEN iris:events:raw:processing, & docker @compose exec -T redis redis-cli LLEN iris:documents:sync:processing, & docker @compose exec -T redis redis-cli LLEN iris:reindex:documents:processing, & docker @compose exec -T redis redis-cli ZCARD iris:approval:interactions:ready, & docker @compose exec -T redis redis-cli ZCARD iris:approval:interactions:processing, & docker @compose exec -T redis redis-cli ZCARD iris:approval:interactions:delayed, & docker @compose exec -T redis redis-cli SCARD iris:approval:interactions:dlq:members)
if ($LASTEXITCODE -ne 0 -or (@($existing + $redisCounts | ForEach-Object { [long]$_ } | Where-Object { $_ -ne 0 }).Count -ne 0)) { throw "Queue or DLQ zero baseline failed; do not continue" }
```
...
```powershell
Invoke-RestMethod -Method Post -Headers $irisHeaders -Uri http://localhost:3000/internal/runtime-control/global -ContentType "application/json" -Body '{"enabled":false}'
foreach ($groupId in $KnownGroupIds + $PilotGroupId) { Invoke-RestMethod -Method Post -Headers $irisHeaders -Uri "http://localhost:3000/internal/runtime-control/groups/$groupId" -ContentType "application/json" -Body '{"enabled":false}' }
Invoke-RestMethod -Method Patch -Headers $irisHeaders -Uri http://localhost:3000/internal/runtime-control/capabilities -ContentType "application/json" -Body '{"generateKnowledgeDrafts":false,"writeKnowledgeBase":false}'
$runtime = Invoke-RestMethod -Headers $irisHeaders -Uri http://localhost:3000/internal/runtime-control/status
if ($runtime.globalEnabled -ne $false -or $runtime.capabilities.generateKnowledgeDrafts -ne $false -or $runtime.capabilities.writeKnowledgeBase -ne $false) { throw "Runtime preflight is not disabled" }
```

## 单群开启和卡片 readiness

仅在前述检查通过后，私下将 `.env.pilot` 的 `IRIS_KNOWLEDGE_CARD_ENABLED=true` 和 `IRIS_KNOWLEDGE_CARD_GROUP_IDS` 设为唯一 `$PilotGroupId`。重建 Core 前确认 allowlist 只有一个值，且不回显私有 env 文件。

重建后，`/internal/status` 的 `knowledgeCards` 只能包含 worker、queue 和 presentation 数字，不能含草稿正文、证据正文、`actorOpenId`、`reason` 或 token。`/internal/approval-interactions/status` 必须在内部 bearer 路由可读，且 `pending`、`processing`、`delayed`、`deadLetter` 都为 `0`。随后只将 `$PilotGroupId` group runtime 设为 `true`，三个 `$KnownGroupIds` 保持 disabled；最后才开启 global runtime 和 Caddy。
